...
import { Calendar, Clock, Search, Download, Database, Settings, Loader, AlertTriangle } from 'lucide-react';
...
/** Normaliza timestamp (API `ts` o `timestamp`) y filtra por deviceId si la fila lo trae. */
function normalizeHistorySourceRow(item, selectedDeviceId) {
  if (!item || typeof item !== 'object') return null;
  if (item.deviceId != null && String(item.deviceId) !== String(selectedDeviceId)) return null;
  const tsRaw = item.timestamp ?? item.ts;
  const rawTs = Number(tsRaw);
  if (!Number.isFinite(rawTs)) return null;
  const props = item.properties;
  const properties =
    props && typeof props === 'object' && !Array.isArray(props) ? { ...props } : {};
  return { ts: rawTs, properties };
}

/** Excluye claves tipo material de sesión LoRa / cifrado del desplegable VALOR. */
const HISTORY_SENSITIVE_PROPERTY_KEY_RE =
  /\b(nwk|app)s?key\b|appskey|nwk_s_key|app_s_key|apps_key|skeyhex|joinnonce|devnonce|appnonce|mic_hex|payload_key\b/i;

function filterSensitivePropertyDescriptors(props) {
  if (!Array.isArray(props)) return [];
  return props.filter(
    (p) =>
      p &&
      p.propertyKey &&
      !HISTORY_SENSITIVE_PROPERTY_KEY_RE.test(String(p.propertyKey)) &&
      !HISTORY_SENSITIVE_PROPERTY_KEY_RE.test(String(p.id || ''))
  );
}
...
const HistoryPage = () => {
    ...
    const loadProperties = async () => {
      ...
      try {
        ...
        const localEntry = (localResp || []).find(
          (d) => d && String(d.deviceId) === String(selectedDeviceId)
        );
        ...
        let props =
          tslResp.data?.data?.properties || tslResp.data?.properties || tslResp.properties || [];
        props = filterSensitivePropertyDescriptors(props);
        ...
        if (props.length === 0) {
          ...
          props = Object.keys(expandedLive)
            .filter(
              (key) =>
                !ignoreKeys.has(key) &&
                !HISTORY_SENSITIVE_PROPERTY_KEY_RE.test(key) &&
    ...
    try {
      ...
      try {
        ...
        if (localData && localData.length > 0) {
          results = localData
            .map((item) => normalizeHistorySourceRow(item, selectedDeviceId))
            .filter(Boolean);
        }
      } catch (e) {
      ...
      const formatted = results
        .map((item) => {
          ...
          if (!Number.isFinite(rawTs)) return null;
          return {
            timestamp: new Date(rawTs).toLocaleString(),
  ...
  const exportToCSV = () => {
    ...
    const rows = historyData.map((item) => [`"${item.timestamp}"`, `"${item.value}"`]);
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const tableData = historyData.map((item) => [item.timestamp, item.value, unit]);
    ...
    <div className="history-page">
      ...
      <div className="filters-bar glass card">
        <div className="filter-group">
          <label>
            <Database size={14} /> {t('history.device_filter')}
          </label>
          <select
        ...
        <div className="filter-group">
          <label>
            <Settings size={14} /> {t('history.value_filter')}
          </label>
          <select
          ...
          >
            {properties.map((p) => (
              <option key={p.id} value={p.propertyKey}>
                {p.name}
              </option>
            ))}
          </select>
        ...
        <div className="filter-group">
          <label>
            <Calendar size={14} /> {t('history.start_filter')}
          </label>
          <input
            type="datetime-local"
            value={startTime}
        ...
        <div className="filter-group">
          <label>
            <Clock size={14} /> {t('history.end_filter')}
          </label>
          <input
            type="datetime-local"
            value={endTime}
        ...
        <div className="chart-container glass card">
          ...
          <div className="chart-wrapper" ref={chartRef}>
            {historyData.length > 0 ? (
              ...
              <div className="empty-state">{loading ? t('history.loading') : t('history.empty')}</div>
            )}
          </div>
        </div>
