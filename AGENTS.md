...
npm test                             # ⚠️ runs ONLY server/test/selfhosted.test.cjs
npm run verify                       # integration smoke (scripts/verify-integration.mjs)

# The other test files in server/test/ are NOT picked up by `npm test`; run them by hand:
node --test server/test/template-sync-plan.test.cjs
node --test server/test/vs133-telemetry-aliases.test.cjs
...
### Backend (`server/`)
`server/server.js` is the monolith (~4.5k lines, ~98 Express route handlers): SSE hub, JWT auth, ingest pipelines, LNS engine glue, and listeners on multiple ports.
...
- OAuth (Google/Microsoft/Yahoo) lives in `server/{google,microsoft,yahoo}-auth-routes.js`, with shared helpers in `server/oauth-shared.js` and provider config in `server/oauth-providers-config.js`. OAuth **only links to pre-existing local users** — it doesn't create accounts. Roles always come from SQLite. Email/password login (`/api/auth/login`) coexists with OAuth; both produce the same JWT.
...
- `docs/` — integration and deploy guides: `DEPLOY-AWS-EC2.md`, `DEPLOY-RENDER.md`, `LNS-SEMTECH-UDP.md`, `SYSCOM-SELFHOSTED.md`, plus per-device notes (`EASTRON-SDM230.md`, `SHENGDA-APP-LAYER-V1.6.md`, `TIMEWAVE-WATER-METER.md`). Read these before changing protocol-level code or deploy scripts.
- `shared/` — cross-tier contracts (`realtime-sse-contract.json`, `lorawan-gateway-bands.json`); changes here ripple to both server and client.
- `.env.example` — canonical list of supported environment variables; check it before introducing a new one.
