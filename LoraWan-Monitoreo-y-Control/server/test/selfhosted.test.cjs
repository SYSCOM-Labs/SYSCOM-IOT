...
test('defaultKeyFromReq uses req.ip/socket and ignores client-sent x-forwarded-for', () => {
  const req = { headers: { 'x-forwarded-for': '203.0.113.5, 10.0.0.1' }, socket: { remoteAddress: '127.0.0.1' } };
  assert.equal(defaultKeyFromReq(req), '127.0.0.1');
});

test('deriveSessionKeys10x matches lora-packet (OTAA MIC / join)', () => {
  const crypto = require('node:crypto');
  const lora_packet = require('lora-packet');
  const { deriveSessionKeys10x } = require('../lorawan-lns-crypto');
  const AppKey = crypto.randomBytes(16);
  const AppNonce = crypto.randomBytes(3);
  const NetId = Buffer.from('000001', 'hex');
  const DevNonce = crypto.randomBytes(2);
  const ours = deriveSessionKeys10x(AppKey, AppNonce, NetId, DevNonce);
  const lib = lora_packet.generateSessionKeys10(AppKey, NetId, AppNonce, DevNonce);
  assert.ok(ours.nwkSKey.equals(lib.NwkSKey));
  assert.ok(ours.appSKey.equals(lib.AppSKey));
});
