...
/** DI cambio intervalo de subida (comando 14h / respuesta corta 94h con cuerpo 4 B). */
const DI_INTERVAL = Buffer.from([0x04, 0x70, 0x01, 0x02]);
/**
 * Trama 94h «intervalo OK»: identificador en aire `3534A337` → sin cifrar `04700102` (igual que DI_INTERVAL).
 * Algunas tablas PDF citan `04500105` para el mismo bloque en aire; no coincide con +33/−33 del firmware de referencia.
 */
const DI_INTERVAL_ACK = Buffer.from([0x04, 0x70, 0x01, 0x02]);
...
/**
 * Palabra de estado 16 bit tras dataUnscramble (TimeWave «State Character» §3.1).
 * Bits 15–8: no usados en la ficha actual (reservados).
 */
function parseStatusWordFromRaw(u16buf) {
  ...
  return {
    raw: w,
    /** Bits 1–0: 00 abierta, 01 cerrada, otro excepción */
    valveOpen: valveBits === 0,
    valveClosed: valveBits === 1,
    valveException: valveBits >= 2,
    /** Bit 2: alimentación (0 normal, 1 baja) */
    lowPowerSupply: Boolean(w & 0x04),
    /** Bit 3: alarma */
    alarm: Boolean(w & 0x08),
    /** Bit 4: sobregiro / overdraft */
    overdraft: Boolean(w & 0x10),
    /** Bit 5: interferencia magnética fuerte */
    strongMagnetic: Boolean(w & 0x20),
    /** Bit 6: force status (0 OFF, 1 ON) */
    forceStatusOn: Boolean(w & 0x40),
    /** Octeto alto (bits 15–8): no usados en ficha actual */
    status_high_byte_reserved: u16buf[0],
  };
}
...
/** Byte de error en trama D4h (§3.2 «Meter Error»). */
function parseErrorByte(b) {
  ...
  return {
    ...
    /** Bit 1: fallo al obtener datos */
    dataRetrieveFailed: Boolean(err & 0x02),
    /** Bit 2: contraseña incorrecta / no autorizado */
    badPassword: Boolean(err & 0x04),
    /** Bit 3: timeout de comunicación del equipo */
...
function parseReading91(dataAfterLen) {
  ...
  /** §2.1.1: −0x33 por byte, orden invertido, × 15,37 → mV (1 B en trama de lectura). */
  const battPlain = dataUnscramble(battEnc);
  const v = battPlain.length ? battPlain.readUInt8(battPlain.length - 1) : 0;
  const batteryMv = Math.round(v * 15.37);

  let batteryPercent = null;
  const pctPlain = dataUnscramble(pctEnc);
  const pu = pctPlain.length ? pctPlain.readUInt8(pctPlain.length - 1) : 0;
