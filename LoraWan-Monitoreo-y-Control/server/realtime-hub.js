 ...
 * Nombres de evento SSE: `shared/realtime-sse-contract.json` (consumidos en `SyscomRealtimeBridge.jsx`).
 */
'use strict';
...
const sseContract = require(path.join(__dirname, '..', 'shared', 'realtime-sse-contract.json'));
...
function createRealtimeHub() {
  ...
  function broadcast(userId, event, payload) {
    ...
    if (event === sseContract.sseTelemetry) metrics.inc('sse_broadcast_telemetry');
    if (event === sseContract.sseLns) metrics.inc('sse_broadcast_lns');
  }
