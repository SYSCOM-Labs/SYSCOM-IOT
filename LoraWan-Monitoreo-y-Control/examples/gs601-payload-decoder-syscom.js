/**
 * GS601 (Milesight) — decoder listo para Syscom IoT
 *
 * Basado en el codec oficial Milesight 2025 (`SensorDecoders/gs-series/gs601/gs601-decoder.js`)
 * con ajustes para la VM de ingesta (`server/payload-decoder.js`):
 * - `decodeUplink({ bytes, fPort })` → `{ data: { ... } }`; `bytes` es siempre un Array<number> en servidor.
 * - Sin `throw` en opcode desconocido ni en `extractBits` / buffer corto (no tumba el uplink).
 * - `readBytes` no avanza el cursor más allá del buffer (evita desbordes y NaN en widgets).
 * - `readUnknownDataType` no lanza.
 * - `daylight_saving_time`: segunda lectura de bits en variable distinta (evita redeclaración confusa).
 * - Batería acotada 0–100 en salida.
 *
 * Pegar en Plantillas → Payload decoder (o device decode config). Opcional: botón «Ajustar» en UI.
 *
 * ---
 * Payload Decoder (original)
 *
...
/* eslint-disable */
// Chirpstack v4 / Syscom IoT
function decodeUplink(input) {
	var bytes = (input && input.bytes != null) ? input.bytes : [];
	if (bytes && !Array.isArray(bytes) && typeof bytes.length === 'number' && typeof bytes.slice === 'function') {
...
function milesightDeviceDecode(bytes) {
	...
	if (!Array.isArray(bytes) && typeof bytes.slice === 'function') {
		bytes = Array.prototype.slice.call(bytes);
	}
	...
	for (counterObj.i = 0; counterObj.i < bytes.length; ) {
		...
		switch (command_id) {
			...
			case 0xc6:
				...
				var bitOptionsEnd = readUInt8(bytes, counterObj, 1);
				// 1:1st, 2: 2nd, 3: 3rd, 4: 4th, 5: last
				decoded.daylight_saving_time.end_week_num = extractBits(bitOptionsEnd, 4, 8);
				// 1：Mon., 2：Tues., 3：Wed., 4：Thurs., 5：Fri., 6：Sat., 7：Sun.
				decoded.daylight_saving_time.end_week_day = extractBits(bitOptionsEnd, 0, 4);
			...
			case 0x5e:
				...
				break;
			default:
				// Opcode no contemplado: no lanzar (la ingesta ignora el decoder si falla la VM).
				break;
		}
	}
	...
	if (result.battery !== undefined && typeof result.battery === 'number') {
		if (result.battery > 100) result.battery = 100;
		if (result.battery < 0) result.battery = 0;
	}

	return result;
}

function readOnlyCommand(allBytes, counterObj, end) {
	return 1;
}
...
function readBytes(allBytes, counterObj, end) {
	...
	var out = allBytes.slice(start, start + n);
	counterObj.i += n;
	return out;
}
...
function readFirmwareVersion(bytes) {
	if (!bytes || bytes.length < 6) {
		...
		var major = bytes[0] & 0xff;
		var minor = (bytes.length > 1 ? bytes[1] : 0) & 0xff;
		return 'v' + major + '.' + minor;
	}
	var major = bytes[0] & 0xff;
	var minor = bytes[1] & 0xff;
...
function readUInt16LE(allBytes, counterObj, end) {
	...
	var value = ((bytes[1] & 0xff) << 8) + (bytes[0] & 0xff);
	return value & 0xffff;
}
...
function readUInt24LE(allBytes, counterObj, end) {
	...
	var value = ((bytes[2] & 0xff) << 16) + ((bytes[1] & 0xff) << 8) + (bytes[0] & 0xff);
...
function readFloat16LE(allBytes, counterObj, end) {
	var bytes = readBytes(allBytes, counterObj, end);
	if (!bytes || bytes.length < 2) return 0;
	var bits = ((bytes[1] & 0xff) << 8) | (bytes[0] & 0xff);
