...
export default function ValueIndicator({
  ...
  const useCustomLabel = valueLabel != null && String(valueLabel).trim().length > 0;
  const textVal = useCustomLabel ? String(valueLabel).trim() : null;
  ...
  if (kind === 'none') {
    return (
      <div className={`vi-none ${compact ? 'vi-none--compact' : ''} ${dark ? 'vi-none--dark' : ''}`}>
        ...
        {subtitle ? (
          <div className="vi-none__sub" style={{ color: subColor }}>
            {subtitle}
          </div>
        ) : null}
        <div className="vi-none__val">
          {textVal ?? fmt(clamped)}
          {unit && !useCustomLabel ? <span className="vi-none__unit">{unit}</span> : null}
        </div>
      </div>
    );
  }
      ...
      <div className={`vi-linear ${compact ? 'vi-linear--compact' : ''} ${dark ? 'vi-linear--dark' : ''}`}>
        {(title || subtitle) && (
          <div className="vi-linear__meta">
            ...
            {subtitle ? (
              <div className="vi-linear__sub" style={{ color: subColor }}>
                {subtitle}
              </div>
            ) : null}
          </div>
        ...
        <div className="vi-linear__val">
          {textVal ?? fmt(clamped)} {!useCustomLabel ? unit : ''}
        </div>
      </div>
    );
  }
      ...
      <div className={`vi-vertical ${compact ? 'vi-vertical--compact' : ''} ${dark ? 'vi-vertical--dark' : ''}`}>
        {(title || subtitle) && (
          <div className="vi-vertical__meta">
            ...
            {subtitle ? (
              <div className="vi-vertical__sub" style={{ color: subColor }}>
                {subtitle}
              </div>
            ) : null}
          </div>
        ...
        <div className="vi-vertical__body">
          ...
          <div className="vi-vertical__val">
            {textVal ?? fmt(clamped)} {!useCustomLabel ? <span>{unit}</span> : null}
          </div>
        </div>
      </div>
    );
      ...
      <div className={`vi-battery ${compact ? 'vi-battery--compact' : ''} ${dark ? 'vi-battery--dark' : ''}`}>
        {(title || subtitle) && (
          <div className="vi-battery__meta">
            ...
            {subtitle ? (
              <div className="vi-battery__sub" style={{ color: subColor }}>
                {subtitle}
              </div>
            ) : null}
          </div>
      ...
      <div className={`vi-compass ${compact ? 'vi-compass--compact' : ''} ${dark ? 'vi-compass--dark' : ''}`}>
        {(title || subtitle) && (
          <div className="vi-compass__meta">
            ...
            {subtitle ? (
              <div className="vi-compass__sub" style={{ color: subColor }}>
                {subtitle}
              </div>
            ) : null}
          </div>
  ...
  return (
    <div className={`vi-numeric ${compact ? 'vi-numeric--compact' : ''} ${dark ? 'vi-numeric--dark' : ''}`}>
      ...
      {subtitle ? (
        <div className="vi-numeric__sub" style={{ color: subColor }}>
          {subtitle}
        </div>
      ) : null}
      <div className="vi-numeric__val">
        {textVal ?? fmt(clamped)} {unit && !useCustomLabel ? <span>{unit}</span> : null}
      </div>
    </div>
  );
}
