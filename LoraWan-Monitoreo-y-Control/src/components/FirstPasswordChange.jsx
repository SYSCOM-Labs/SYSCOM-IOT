...
export default function FirstPasswordChange() {
  ...
  const p1PolicyOk = useMemo(() => isPasswordPolicySatisfied(p1), [p1]);
  const p1BorderClass = useMemo(() => {
    if (!p1) return '';
    return p1PolicyOk ? 'input-border--valid' : 'input-border--invalid';
  }, [p1, p1PolicyOk]);
  const p2BorderClass = useMemo(() => {
    if (!p2) {
      if (p1PolicyOk) return 'input-border--invalid';
      return '';
    }
    if (p1PolicyOk && p1 === p2) return 'input-border--valid';
      ...
      <div className="login-card login-card--premium glass card" style={{ maxWidth: 480 }}>
        ...
        <form onSubmit={onSubmit} className="login-form">
          <div className="form-group">
            ...
            <input
              type="password"
              className={['glass', p1BorderClass].filter(Boolean).join(' ')}
              value={p1}
          ...
          <div className="form-group">
            ...
            <input
              type="password"
              className={['glass', p2BorderClass].filter(Boolean).join(' ')}
              value={p2}
