...
- **Eventos:** los tipos con payload (`sseTelemetry`, `sseLns` en el JSON) están en `shared/realtime-sse-contract.json` (mismo contrato que usa el front en `EventSource`).
  - `connected` — conexión establecida (fijo en el servidor).
  - **Telemetría** (`sseTelemetry`, p. ej. `telemetry`) — datos: `{ deviceId, deviceName, timestamp }` tras guardar telemetría (ingesta HTTP, LNS, POST `/api/telemetry`). Se notifica a cada `userId` que recibe fila (propietario + cuentas con el mismo dispositivo asignado).
  - **LNS UI** (`sseLns`, p. ej. `lns`) — `{ id, eventType, devEui, meta, createdAt }` (p. ej. `downlink_sent`, `downlink_device_acked`).
