...
| `SYSCOM_LNS_PULL_RESP_ON_PUSH` | **`1`** (defecto): tras un uplink (`PUSH_DATA`) envía `PULL_RESP` al último peer de **PULL_DATA** si sigue fresco. **`0`**: solo en el próximo keepalive. Necesario para clase A (RX1 1–5 s vs keepalive ~10 s). |
| `SYSCOM_LNS_PULL_PEER_MAX_AGE_MS` | Antigüedad máxima del peer PULL cacheado (ms), defecto **70000**. |
