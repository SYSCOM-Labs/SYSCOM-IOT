...
async function main() {
  ...
  const proc = spawn(process.execPath, ['server/server.js'], {
    ...
    env: {
      ...
      SYSCOM_LNS_MAC: '0',
      /** Evitar EADDRINUSE si el puerto UDP 1700 ya está ocupado en el host. */
      LNS_UDP_PORT: '0',
    },
  ...
  try {
    ...
    assert(hist.status === 200 && Array.isArray(hist.data), 'history');
    assert(
      hist.data.length >= 1 &&
        hist.data[0].timestamp != null &&
        hist.data[0].properties &&
        typeof hist.data[0].properties === 'object',
      'historial con al menos un punto (timestamp + properties)'
    );
    ...
    assert(login.status === 200 && login.data.token, 'login admin');
    let adminToken = login.data.token;

    login = await req('POST', `${base}/api/auth/login`, {
      body: { email: 'verify-user@test.local', password: 'VerifyPass1!' },
    });
    assert(login.status === 200 && login.data.token, 'login user');
    let userToken = login.data.token;

    /** Las cuentas creadas por staff llevan mustChangePassword; sin esto GET /api/devices devuelve 403. */
    const admFp = await req('POST', `${base}/api/auth/first-password`, {
      token: adminToken,
      body: { newPassword: 'VerifyPass1!' },
    });
    assert(admFp.status === 200 && admFp.data.token, `admin first-password ${admFp.status}`);
    adminToken = admFp.data.token;

    const usrFp = await req('POST', `${base}/api/auth/first-password`, {
      token: userToken,
      body: { newPassword: 'VerifyPass1!' },
    });
    assert(usrFp.status === 200 && usrFp.data.token, `user first-password ${usrFp.status}`);
    userToken = usrFp.data.token;
